...
const AppointmentBookingModal = ({ isOpen, onClose, onBook, showToast }) => {
    ...
    const fetchAvailableSlots = async () => {
        try {
            const response = await fetch(`/api/v1/appointment-booking/available_slots/?doctor_id=${selectedDoctor}&date=${selectedDate}`);
            const data = await response.json();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Map appointment types to backend choices
            const typeMapping = {
            ...
            // Prepare data matching the serializer fields
            const bookingData = {
                doctor_id: parseInt(selectedDoctor),
                appointment_date: selectedDate,
                start_time: convertTimeFormat(selectedTime),
                appointment_type: typeMapping[appointmentType] || 'consultation',
                patient_notes: notes || ''
            };

            const response = await fetch('/api/v1/appointment-booking/book/', {
                method: 'POST',
                headers: {
